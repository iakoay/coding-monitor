...
class WebviewPanel {
    ...
    loadConfigValues() {
        const cfg = vscode.workspace.getConfiguration();
        return {
            codingPlan_minimaxKey: cfg.get('codingPlan.minimaxKey', ''),
            codingPlan_glmKey: cfg.get('codingPlan.glmKey', ''),
            claudeContext_refreshInterval: cfg.get('claudeContext.refreshInterval', 5000),
            claudeContext_showPercentage: cfg.get('claudeContext.showPercentage', true),
            claudeContext_warningThreshold: cfg.get('claudeContext.warningThreshold', 70),
            claudeContext_criticalThreshold: cfg.get('claudeContext.criticalThreshold', 90),
            claudeContext_enableNotifications: cfg.get('claudeContext.enableNotifications', true),
    ...
    getHtml(state, warningThreshold, criticalThreshold) {
...
<body>
    ...
    getClaudeTabHtml(info, warningThreshold, criticalThreshold) {
        if (!info) {
        ...
        const categoryRows = info.categories.map(c => `<tr><td>${c.name}</td><td>${(0, helpers_1.formatTokens)(c.tokens)}</td><td>${c.percentage}</td></tr>`).join('\n');
        return `
        <div class="info-card">
            <div class="info-row">
    ...
    getApiTabHtml(minimax, glm) {
        ...
        <div class="api-card">
            ...
            <div class="info-row">
                ...
                <span class="value">${minimax.h5RemainCount}（${(0, helpers_1.formatDuration)(minimax.h5Remain)} 后重置）</span>
            </div>
            <div class="info-row">
        ...
        <div class="api-card">
            ...
            <div class="info-row">
                ...
                <span class="value">${glm.tokensWeek}%（${(0, helpers_1.formatResetTime)(glm.tokensWeekReset)} 后重置）</span>
            </div>
            <div class="info-row">
